...
public class HRDashboard extends JFrame {
    ...
    private void handlePrintTimeCard(String empId) {
        try {
            ...
            params.put("periodEndDate", "07/15/2026"); // Or get this from a date picker if you have one
            
            // 2. Call the updated ReportGenerator using the database connection
            // Ensure "timecard.jasper" is in your src/reports/ directory
            reports.ReportGenerator.generateReport("/reports/timecard.jasper", params);
            
        } catch (Exception ex) {
            ex.printStackTrace();
            JOptionPane.showMessageDialog(this, "Error generating time card: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
