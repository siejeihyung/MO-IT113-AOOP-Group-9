...
public class FinanceDashboard extends JFrame {
    ...
    private void handlePrintIndividualPayslip() {
        ...
        try {
            // Create the parameters map for the Jasper report
            Map<String, Object> parameters = new HashMap<>();
            parameters.put("employeeId", empId);
            parameters.put("companyName", "MotorPH");
            
            // Call the refactored ReportGenerator (which now uses DB connection)
            // Ensure the path matches where you placed the .jasper file in src/
            reports.ReportGenerator.generateReport("/reports/employeePayslip.jasper", parameters);
            
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, "Error printing payslip: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        }
    }

    private void handlePrintPayrollSummary() {
        try {
            // For the Summary, you might not need an ID, just global parameters
            Map<String, Object> params = new HashMap<>();
            params.put("companyName", "MotorPH");
            params.put("periodEndDate", "07/15/2026");

            // Call the refactored ReportGenerator
            reports.ReportGenerator.generateReport("/reports/PayrollSummary.jasper", params);
            
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, "Failed to generate summary: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
