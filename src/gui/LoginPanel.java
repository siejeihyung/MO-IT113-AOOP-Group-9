...
public class LoginPanel extends JFrame {
    ...
    private void checkLogin() {
        ...
        // 1. First, try the standard system authentication (Admin, HR, Finance)
        String role = employeeService.authenticate(user, pass);

        // 2. If system login fails, try Employee authentication (EmployeeID + LastName)
        if (role == null) {
            // Using your EmployeeDAO to check if ID and LastName match
            EmployeeDAO empDao = new EmployeeDAO();
            // This assumes findById returns an Employee object
        ...
        if (role != null) {
                ...
                SwingUtilities.invokeLater(() -> {
                    try {
                        ...
                        switch (finalRole.toUpperCase()) {
                            case "ADMIN" -> new AdminDashboard(finalUser);
                            case "HR" -> new HRDashboard(finalUser);
            ...
            // ... (keep your existing error/lockout logic)
            attempts++;
