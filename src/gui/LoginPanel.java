...
public class LoginPanel extends JFrame {
    ...
    private void checkLogin() {
        ...
        // Check if fields are empty BEFORE running authentication logic
        if (user.isEmpty() || pass.isEmpty()) {
            feedbackLabel.setForeground(Color.RED);
            feedbackLabel.setText("Username and Password are required.");
            
            // Highlight the empty field to help the user
            if (user.isEmpty()) {
                usernameField.requestFocus();
            } else {
                passwordField.requestFocus();
            }
            return; // Exit the method so it doesn't count as a failed attempt
        }
        // --- NEW VALIDATION END ---
